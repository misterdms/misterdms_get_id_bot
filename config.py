...
ИСПРАВЛЕНО: Порядок объявления переменных, безопасная обработка MESSAGES
"""
...
# НОВЫЕ ФИЧИ v4.1 - КОМАНДЫ СВЯЗИ (ПЕРЕНЕСЕНО ВВЕРХ!)
...
# Функция для создания MESSAGES (ИСПРАВЛЕНО: теперь создается после объявления переменных)
def create_messages():
    """Создание сообщений с правильными переменными"""
    return {
...
# Создаем MESSAGES после объявления всех переменных
...
__all__ = [
    ...
    'API_ID_PATTERN', 'API_HASH_PATTERN'
]
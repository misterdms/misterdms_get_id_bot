...
ИСПРАВЛЕНО: Правильный порядок объявления переменных, безопасная обработка MESSAGES
"""
...
# Проверка DATABASE_URL на примеры
if DATABASE_URL and ('user:password@host' in DATABASE_URL or 
...
# === АДМИНСКИЕ КОНТАКТЫ (ОБЪЯВЛЯЕМ РАНЬШЕ ДЛЯ MESSAGES!) ===
...
# === ВАЛИДАТОРЫ ===
API_ID_PATTERN = r'^\d{7,8}$'
API_HASH_PATTERN = r'^[a-f0-9]{32}$'
...
# === СООБЩЕНИЯ БОТА (СОЗДАЕМ ПОСЛЕ ОБЪЯВЛЕНИЯ ВСЕХ ПЕРЕМЕННЫХ!) ===
MESSAGES = {
...
__all__ = [
    ...
    'API_ID_PATTERN', 'API_HASH_PATTERN', 'PORT', 'RENDER_EXTERNAL_URL',
    'WEBHOOK_SECRET', 'ALERT_TELEGRAM_CHAT', 'ALERT_ADMIN_IDS'
]
